...
const MatchesHoverCard = ({ match }) => {
  ...
  const { competition1, competition2, score, date, time, stadium } = match;
    ...
    <div>
      ...
      <div className='mycard border-b-2 text-white border-white flex justify-evenly w-full h-20 items-center hover:border-b-gGlow transition-all duration-1000 cursor-pointer hover:bg-gray-900'>
        <div className='mycard-1 underline text-sm hover:text-gGlow duration-700'>
          <p onClick={()=>Navigate('/footballgame')} >Create <br /> Team</p>
        </div>
        ...
        <div className='mycard-4 score-div'>{score}</div>
        <div className='mycard-5 date-div'>{date}</div>
        <div className='mycard-5 time-div'>{time}</div>
        <div className='mycard-5 stadium-div'>{stadium}</div>
      </div>
    </div>
  );
};
